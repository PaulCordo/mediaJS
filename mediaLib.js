...
var mediaLibJS = {};
(function() {
  var mediaReady = document.createEvent('Event'),
  ...
  var gallery = function(media) {
    ...
    intVideo = function(media) {
      ...
      player.loadMedia = changeSource;
      player.stopMedia = function(){
        if(!videoElement.paused) play();
        window.clearTimeout(loaderTimeOutID);
    ...
    extVideo = function(media) {
      // iframe configuration
      var playerElement = this.element = document.createElement('iframe'),
        player = this;
      playerElement.id = media.provider + media.id;
      playerElement.provider = media.provider;
      playerElement.className = "ext-video";
      ...
      switch (media.provider) {
        case 'youtube':
          playerElement.setAttribute('src', 'https://youtube.com/embed/' + media.id + '?enablejsapi=' + (media.api || '1') + '&playerapiid=' + playerElement.id + '&rel=' + (media.rel || '0') + '&showinfo=' + (media.showinfo || '0') + '&color=' + (media.color || 'white') + '&iv_load_policy=' + (media.ivLoadPolicy || '3') + '&disablekb=' + (media.disablekb || '1') + '&cc_load_policy=' + (media.ccLoadPolicy || '0'));
          playerElement.addEventListener('load', function(e) {
            player.post('listening');
          });
          playerElement.addEventListener('ready', function(e) {
            player.post('command', 'setPlaybackQuality', ['highres']);
          });
          break;
        case 'vimeo':
          playerElement.setAttribute('src', 'https://player.vimeo.com/video/' + media.id + '?api=' + (media.api || '1') + '&player_id=' + playerElement.id + '&title=' + (media.title || '0') + '&portrait=' + (media.portrait || '0') + '&color=' + (media.color || 'f0f0f0') + '&byline=' + (media.byline || '0') + '&badge=' + (media.badge || '0'));
          playerElement.addEventListener('ready', function(e) {
      ...
      function keyboardControls(event) {
        ...
        switch (event.key) {
          case " ":
            ...
            break;
          case "Escape":
            playerElement.pauseMedia();
            playerElement.dispatchEvent(mediaPaused);
            break;
        }
      }
      // Create external API
      playerElement.playMedia = function() {
        player.post('play');
        window.addEventListener('keypress', keyboardControls);
      };
      playerElement.pauseMedia = function() {
        player.post('pause');
        window.removeEventListener('keypress', keyboardControls);
      };
    };
  extVideo.playerOrigin = {
    "vimeo": "*",
    "youtube": "*"
  };
  extVideo.statesLib = {
  ...
  function onMessageReceived(event) {
    ...
    var data = JSON.parse(event.data),
      ...
      states = extVideo.statesLib[origin];
    if (extVideo.playerOrigin[origin] === "*") extVideo.playerOrigin[origin] = event.origin;
  ...
  extVideo.prototype.post = function(first, second, third) {
    var data = {},
    ...
    this.element.contentWindow.postMessage(message, extVideo.playerOrigin[origin]);
  };
  mediaLibJS.gallery = gallery;
  mediaLibJS.intVideo = intVideo;
  mediaLibJS.extVideo = extVideo;
}());